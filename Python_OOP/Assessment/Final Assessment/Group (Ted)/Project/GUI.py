from tkinter import *
from Bank import BankAccountTasks#Used to call methods in the class class of the Bank 用于调用Bank的class类中的方法
root = Tk()
root.title("Bank Account System")#title标题
root.geometry('700x570')# Window size窗体大小

topFrame = Frame(root, width='70', height='35', pady='20')# To determine the size框架
...
myEntry = Text(topFrame, width='70', height='15')#Text文本

scroll = Scrollbar(topFrame)#滚动条up or down
scroll.pack(side=RIGHT, fill=Y)
myEntry.configure(yscrollcommand=scroll.set)#变形
...
def show_account_with_most_money():#展示钱最多的人
    myEntry.delete('0.0', 'end')
...
def show_account_with_most_transactions():#展示最多的操作的客户
    myEntry.delete('0.0', 'end')
